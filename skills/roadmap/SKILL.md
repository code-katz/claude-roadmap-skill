---
name: roadmap
allowed-tools: Bash(git add:*), Bash(git commit:*), Bash(git push:*), Bash(git rev-parse:*)
...
**Delegation:** With claude-team v0.7+ installed, roadmap updates can also be delegated to the River subagent ("have River update the roadmap") without switching the session's own persona. The same read-update-history-approve workflow applies inside the delegated agent.
...
Commit on the CURRENT branch. Never run `git checkout` or `git switch`; on a feature or session branch, the roadmap update reaches the default branch through the project's normal merge/PR flow. If a pre-commit hook rejects a commit on main/master (claude-team's branch guard does this by design), do not bypass it; move to a registered branch instead.
